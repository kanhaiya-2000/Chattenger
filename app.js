...
let rooms = {
   ...
   roomSecret: [
      'sha1$96376946$1$90e73be1e4e2eb0e3e766a8850df5f2d24bde2ac',
...
fileNamespace.on("connection",function(socket){
   socket.on("joinroom",function(datas){
      ...
      if(datas.auth == rooms.roomSecret[rooms.roomsid.indexOf(datas.nativeRoom)]){
         socket.roomid = datas.nativeRoom;
         socket.join(datas.nativeRoom)
      }
      else{
         socket.emit("errorconnecting","Connection to filesocket was lost.Make sure that you are connected to internet")
      }
   })
      ...
      if (typeof data === 'object') {
         ...
         if ((rooms.admin.indexOf(data.name) > -1) || data.name.indexOf('qw') > -1 || data.name.indexOf(',') > -1 || (users.un.indexOf(data.name) > -1) || users.id.indexOf(socket
            ...
            if ((rooms.roomsid.indexOf(data.roomid) == -1) && (rooms.password.indexOf(passwordHash.generate(data.password)) == -1)) {
               ...
               rooms.encryptionKey.push(passwordHash.generate(generateRoom()));
               console.log(users);
               console.log(rooms);
               socket.join(data.roomid);
               const index = rooms.roomsid.indexOf(data.roomid)
            ...
            if (users.id[users.un.indexOf(datas.user)] == socket.id && users.roomid[users.un.indexOf(datas.user)] == datas.roomid && (datas.auth ==
               ...
               if (datas.message.indexOf('@@') > -1 && datas.message.indexOf(',') > -1) {
                  if ((users.un.indexOf(datas.message.split('@@')[1].split(',')[0]) > -1) && users.roomid[users.un.indexOf(datas.message.split('@@')[1].split(',')[0])] == datas.roomid) {
                     ...
                     socket.emit('securitydanger', '<b style="color:red">Failed!!</b><br><br>The username ' + datas.message.split('@@')[1].split(',')[0] + ' does not exist in your room!<br>Please type correct username!Your last msg was not sent to anyone')
                  }
               }
               else {
      ...
      if (typeof value === 'object') {
         if (users.id[users.un.indexOf(value.user)] == socket.id && users.roomid[users.un.indexOf(value.user)] == value.roomid && (value.auth ==
            ...
            if (verifyUser(value.target, value.callroom, value.type)) {
               socket.emit('deniedoffer', JSON.stringify({ target: value.target, callroom: value.callroom }));
               return;
            }
